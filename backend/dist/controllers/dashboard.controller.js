...
const getStats = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        payments.forEach(p => {
            if (p.status === client_1.PaymentStatus.COMPLETED)
                paymentStats.completed = p._count._all;
            else if (p.status === client_1.PaymentStatus.PENDING)
                paymentStats.pending = p._count._all;
            else if (p.status === client_1.PaymentStatus.FAILED)
                paymentStats.failed = p._count._all;
            else if (p.status === client_1.PaymentStatus.REFUNDED)
        ...
        const todayPayments = yield PrismaInstance_1.default.payment.findMany({
            where: { created_at: { gte: startOfToday }, status: client_1.PaymentStatus.COMPLETED },
            select: { created_at: true, amount: true }
        });
